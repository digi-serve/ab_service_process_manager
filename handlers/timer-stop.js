...
module.exports = {
    ...
    */
   key: "process_manager.timer-stop",
    ...
    *        api_sails/api/controllers/process_manager/timer-stop.
    * @param {fn} cb
    *        a node style callback(err, results) to send data when job is finished
    */
   fn: function handler(req, cb) {
      req.log("process_manager.timer-stop:");
      ...
      ABBootstrap.init(req)
         .then((AB) => { // eslint-disable-line
            ...
            if (timer) ActiveTimer.stop(req, timer.id);
            else {
         ...
         .catch((err) => {
            req.notify.developer(err, {
               context:
                  "Service:process_manager.timer-stop: Error initializing ABFactory",
            });
            cb(err);
         });
   },
